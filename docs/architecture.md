...
- 追記（2026-09-23）: macOS 27.0 の実機で `contextSize` を取得すると 8192 だった。公式ドキュメントの 4096 とは異なり、決め打ちしない方針の正しさを裏付けている。
...
- トレードオフ: `Transcript` のシリアライズ形式は Foundation Models フレームワーク側の内部表現に依存する。OS ・モデルのバージョンが上がった際に、保存済みデータのデコードやセッション復元に失敗する可能性がある（Apple 公式ドキュメントで明示的な後方互換保証は確認できていない、TBD、#9）。失敗した場合は保存データを破棄し、空の会話から始める形でリカバリする方針とする。
...
- テスト方針: TBD（#4）。ユーザーからは「ビルドが通ることの確認」と「SwiftUI プレビューでの見た目確認」のみ指示があり、自動テスト（Unit Test / UI Test）の要否・範囲は未確定。
